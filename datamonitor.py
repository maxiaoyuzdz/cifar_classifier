...
parser.add_argument('-ll', '--log_list', type=str2array, nargs='?', default="q1.log",
                    help="input log name")
parser.add_argument('-xx', '--x_list', type=str2array, nargs='?', default="1",
                    help="input x values spilt by ,")
...
def drawParaterCompareGraphic():
    print(Y_train_data)
    print('===')
    print(Y_validation_daya)
    print('===')
    print(X_data)
    # draw graphic
    plt.figure()
    ...
    t_loss_line, = plt.plot([])
    t_loss_line.set_xdata(X_data)
    t_loss_line.set_ydata(Y_train_data)
    v_loss_line, = plt.plot([])
    v_loss_line.set_xdata(X_data)
    v_loss_line.set_ydata(Y_validation_daya)

    min_x = min(X_data)
    max_x = max(X_data)

    plt.xlim(min_x, max_x)
    plt.ylim(0, max(max(Y_train_data)+1, max(Y_validation_daya)+1))
    plt.xlabel('prameter')
    plt.ylabel('loss')
    plt.title('parameter compare')

    loss_name_arr = ['train', 'val']
...
def parameterComparePattern():
    for log_file in args.log_list:
        if checkfileexists(args.log_dir + log_file):
            ...
            compare_training_accuracy_data.append(training_accuracy_data)
            compare_validation_accuracy_data.append(validation_accuracy_data)
            max_train_loss = np.max(training_loss_data)
            max_validation_loss = np.max(validation_loss_data)
            Y_train_data.append(max_train_loss)
            Y_validation_daya.append(max_validation_loss)

    for data in args.x_list:
        X_data.append(float(data))

    drawParaterCompareGraphic()
...
def main():
    ...
    # draw parameter graphic
    if args.work_type == 'p':
        parameterComparePattern()
